...
const ImageModal = ({ isCircular, selfData, handleEditFunc }) => {
    ...
    return (
        <div className='p-5 relative flex items-center flex-col h-full'>
            ...
            <label htmlFor='btn-submit' className='absolute bottom-10 left-0 p-3 bg-gray-700 text-white rounded-2xl cursor-pointer'>Upload</label>
...

            {
                ...
                loading ? <Box sx={{ display: 'flex' }}>
                    <CircularProgress />
                </Box> : <div className='absolute bottom-10 right-0 p-3 bg-gray-700 text-white rounded-2xl cursor-pointer' onClick={handleSubmitBtn}> Submit</div>
            }
        </div>
    )
}
